...
class App(ctk.CTk):
    ...
    def __init__(self):
        ...
        self.geometry("1100x750") # Zvětšena šířka a výška
        ...
        self.file_checkbox_vars = {}  # Slovník pro proměnné checkboxů {filepath: BooleanVar}
        self.file_info_data = {}      # Slovník pro uložení načtených informací {filepath: info_dict}

        # --- Horní Frame (Import, Info, Správa) ---
        self.frame_top = ctk.CTkFrame(self)
        self.frame_top.pack(pady=(10,0), padx=10, fill=X)
        # Grid layout pro horní panel
        self.frame_top.grid_columnconfigure(0, weight=0) # Tlačítko Import
        self.frame_top.grid_columnconfigure(1, weight=1) # Label Počet souborů (roztáhne se)
        self.frame_top.grid_columnconfigure(2, weight=0) # Tlačítko Obnovit info (ikona)
        self.frame_top.grid_columnconfigure(3, weight=0) # Tlačítko Správa konfigů
        ...
        # Tlačítko Obnovit info (s ikonou)
        self.button_refresh_info = ctk.CTkButton(
            self.frame_top,
            text="🔄", # <<< IKONA ZDE
            font=ctk.CTkFont(size=18), # Velikost ikony - upravte podle potřeby
            command=self.refresh_all_file_info,
            state="disabled", # Povolí se, až budou soubory
            width=40,  # Menší šířka pro ikonu
            height=28, # Můžeme přizpůsobit výšku ostatním tlačítkům
            fg_color="transparent", # Volitelné: průhledné pozadí
            border_width=1,         # Volitelné: rámeček
            text_color="gray60"      # Volitelné: barva ikony
        )
        self.button_refresh_info.grid(row=0, column=2, padx=(10, 5), pady=5, sticky="e")

        # Tlačítko Správa Konfigurací
        ...
        self.button_manage_configs.grid(row=0, column=3, padx=(0, 0), pady=5, sticky="e")
        ...
        self.files_info_frame = ctk.CTkScrollableFrame(self, height=150)
        self.files_info_frame.pack(pady=5, padx=10, fill=BOTH, expand=True)
        ...
        self.button_save_csv = ctk.CTkButton(self.frame_save, text="CSV (,)", command=lambda: self.save_batch_results('csv', decimal_separator=','), state="disabled", width=90)
        self.button_save_csv.pack(side=LEFT, padx=5)
        self.button_save_csv_dot = ctk.CTkButton(self.frame_save, text="CSV (.)", command=lambda: self.save_batch_results('csv', decimal_separator='.'), state="disabled", width=90)
        self.button_save_csv_dot.pack(side=LEFT, padx=5)
        self.button_save_xlsx = ctk.CTkButton(self.frame_save, text="XLSX", command=lambda: self.save_batch_results('xlsx'), state="disabled", width=80)
        self.button_save_xlsx.pack(side=LEFT, padx=5)

        # --- Frame pro info o průběhu ---
        ...
        self.frame_bottom.pack(pady=(0, 10), padx=10, fill=BOTH, expand=True)
...
# KONEC METODY __init__
    ...
    def refresh_all_file_info(self):
        """Znovu načte informace pro všechny aktuálně vybrané soubory
           a překreslí seznam (kartičky)."""
        if not self.selected_file_paths:
            self.log_message("INFO: Není co obnovovat, žádné soubory nejsou vybrány.")
            return

        count = len(self.selected_file_paths)
        self.log_message(f"INFO: Obnovuji informace pro {count} souborů...")
        logger.info(f"refresh_all_file_info: Zahajuji obnovu informací pro {count} souborů.")

        # Zobrazíme indikátor načítání
        self.label_file_count.configure(text=f"Vybráno souborů: {count}. Obnovuji informace...")
        self.label_progress_info.configure(text="Analyzuji soubory...")
        self.progressbar.configure(mode="indeterminate") # Neurčitý progress bar
        self.progressbar.start()
        self.update_idletasks() # Zobrazíme změny hned

        # Zakážeme tlačítka během obnovy
        try:
            self.button_import.configure(state="disabled"); self.button_process.configure(state="disabled")
            self.button_refresh_info.configure(state="disabled"); self.button_manage_configs.configure(state="disabled")
            self.button_save_csv.configure(state="disabled"); self.button_save_xlsx.configure(state="disabled")
        except Exception as e: logger.warning(f"Chyba při zakazování tlačítek během refresh: {e}")

        # --- Znovu načteme informace pro všechny soubory ---
        self.file_info_data = {} # Vyčistíme stará data
        ...
        for file_path in self.selected_file_paths:
             ...
             try:
                  info = self._get_file_info(file_path) # Voláme get_info
                  self.file_info_data[file_path] = info # Uložíme výsledek
                  if info.get("error"): any_errors_during_info = True
             except ConfigError as ce: # Zachytíme chybu konfigu ZDE
                  fname = os.path.basename(file_path); logger.error(f"Chyba konfigurace pro '{fname}': {ce}", exc_info=False)
                  ...
                  fname = os.path.basename(file_path); logger.error(f"Chyba v _get_file_info pro '{fname}': {e}", exc_info=True)
                  any_errors_during_info = True
        ...
        # --- Překreslíme UI s novými informacemi ---
        self.label_progress_info.configure(text="")
        # Zavoláme metodu, která vykreslí kartičky na základě self.file_info_data
        self._update_file_info_ui() # Tato metoda už správně nastaví labely a tlačítka

        # Zastavíme progress bar
        self.progressbar.stop(); self.progressbar.set(0); self.progressbar.configure(mode="determinate")

        # Povolíme tlačítka (kromě Save, které řeší _update_file_info_ui/_enable_save)
        try:
            self.button_import.configure(state="normal"); self.button_manage_configs.configure(state="normal")
            # Stav Process a Refresh tlačítka se nastavil v _update_file_info_ui
        except Exception as e_ui: logger.warning(f"Chyba povolování UI po refresh: {e_ui}")

        self.log_message(f"INFO: Informace o souborech obnoveny.")
        logger.info("refresh_all_file_info: Dokončeno.")

# KONEC METODY refresh_all_file_info
...
# UVNITŘ TŘÍDY App(ctk.CTk):
    ...
    def _reset_selection_and_ui(self):
        ...
        # UVNITŘ _reset_selection_and_ui

        # Zakážeme tlačítka
        if hasattr(self, 'button_process'): self.button_process.configure(state="disabled")
        if hasattr(self, 'button_refresh_info'): self.button_refresh_info.configure(state="disabled") # <<< PŘIDÁNO ZDE
        # Tlačítko calc_stats už neexistuje
...
# UVNITŘ TŘÍDY App(ctk.CTk):
    ...
    # Metoda select_files - Verze volající refresh_all_file_info
    def select_files(self):
        """Otevře dialog pro výběr souborů, PŘIDÁ je k seznamu
           a spustí obnovení informací."""
        ...
        filepaths_tuple = filedialog.askopenfilenames(
            title='Vyberte soubor(y) k přidání do seznamu',
            initialdir=initial_dir_import,
            filetypes=ftypes,
            parent=self
            )
        ...
        for fp in new_filepaths:
            if fp not in current_paths_set: self.selected_file_paths.append(fp); current_paths_set.add(fp); added_count += 1
        ...
        if added_count > 0:
            ...
            self.log_message(f"INFO: Přidáno {added_count} souborů. Celkem: {len(self.selected_file_paths)}. Obnovuji informace...")
            # <<< Zavoláme refresh, který načte info a překreslí UI >>>
            self.refresh_all_file_info()
            # <<< KONEC VOLÁNÍ REFRESH >>>
        else:
...
# KONEC METODY select_files
    ...
    def _update_file_info_ui(self):
        ...
        # Vyčistíme staré widgety
        for widget in self.files_info_frame.winfo_children(): widget.destroy()
        self.file_info_widgets = {}; self.file_checkbox_vars = {} # Vyčistíme cache

        if not self.selected_file_paths: self._reset_selection_and_ui(); return

        count = len(self.selected_file_paths)
        # Získáme počet chyb z PŘEDEM načtených dat v self.file_info_data
        files_with_errors = sum(1 for info in self.file_info_data.values() if info.get("error"))
        valid_files_count = sum(1 for info in self.file_info_data.values() if info.get("is_valid"))

        # Aktualizujeme label s počtem souborů
        count_text = f"Vybráno souborů: {count}" + (f" (Problémy u {files_with_errors})" if files_with_errors > 0 else "")
        self.label_file_count.configure(text=count_text)
        # self.log_message(f"INFO: {count_text}") # Nyní logujeme až na konci
        logger.info(f"Zobrazuji info pro {count} souborů.")
        # self.update_idletasks() # Není potřeba zde, dělá se v cyklu níže? Raději ne.

        # --- Vytvoření "kartiček" ---
        for file_path in self.selected_file_paths:
            # Získáme PŘEDEM načtená data
            info = self.file_info_data.get(file_path)

            if info is None: # Pokud data z nějakého důvodu chybí v cache
                 logger.error(f"Chybí načtená data pro {file_path}! Vytvářím chybovou kartičku.")
                 # Vytvoříme chybovou kartičku manuálně
                 info = { "filename": os.path.basename(file_path), "config_name": "CHYBA INFO", "config_path": None,
                          "row_count": "N/A", "stats_known": "N/A", "stats_unknown": "",
                          "error": "Chybí data v cache", "config_color": "red", "is_valid": False }
                 files_with_errors = files_with_errors + 1 if file_path not in [fp for fp, inf in self.file_info_data.items() if inf.get("error")] else files_with_errors # Zvýšíme počet chyb, pokud ještě není započítán
                 valid_files_count = valid_files_count -1 if info.get("is_valid") else valid_files_count # Snížíme počet validních
            ...
            # Případné označení chyby na rámečku (pokud je chyba, ale je validní?? To by nemělo nastat)
            if not info["is_valid"] and info.get("error"):
                 try: file_frame.configure(border_color=info["config_color"]) # Použijeme barvu z info
                 except KeyError: pass
            elif info["is_valid"]:
                 valid_files_count += 1 # Počítáme validní až zde
                 if info.get("error"): files_with_errors += 1 # Může být validní, ale s chybou? Ne.

        # --- Dokončení aktualizace UI ---
        # Aktualizujeme finální text počtu souborů po projetí všech
        final_count_text = f"Vybráno souborů: {count}" + (f" (Problémy u {files_with_errors})" if files_with_errors > 0 else "")
        self.label_file_count.configure(text=final_count_text)

        # Povolení Zpracovat, pokud je alespoň JEDEN validní soubor ZAŠKRTNUTÝ
        any_checked_valid = any(var.get() for var in self.file_checkbox_vars.values())
        process_state = "normal" if any_checked_valid else "disabled"
        self.button_process.configure(state=process_state)

        # Povolíme tlačítko Obnovit, pokud máme vybrané soubory
        self.button_refresh_info.configure(state="normal" if self.selected_file_paths else "disabled")
